...
const BsHeroWrap = styled.div`
  ...
  position: relative;
  overflow: hidden;
  border-bottom: 1px solid rgba(230,48,24,.1);
  background: #060606;
...
const BsHeroGlow = styled.div`
  position: absolute;
  inset: 0;
  background: radial-gradient(ellipse 80% 60% at 55% 40%, rgba(230,48,24,0.05), transparent 65%);
  pointer-events: none;
`;
const BsHeroScan = styled.div`
  ...
  height: 1px;
  z-index: 2;
  background: linear-gradient(90deg, transparent, ${T.bs} 40%, ${T.bs} 60%, transparent);
  opacity: 0.05;
...
const BsHeroLeft = styled.div`
  ...
  border-right: 1px solid rgba(230,48,24,.06);
  @media (max-width: 768px) { padding: 100px 20px 60px; border-right: none; }
`;
...
const BsH1 = styled.h1`
  ...
  em { font-style: italic; color: ${T.bs}; }
`;
...
const BsPrice = styled.span`
  ...
  sup { font-family: ${T.ffMono}; font-weight: 300; font-size: 8px; color: ${T.bs}; margin-right: 1px; }
`;
...
const BsHonest = styled.div`
  ...
  gap: 14px;
  background: rgba(230,48,24,.05);
  border: 1px solid rgba(230,48,24,.18);
...
const CtaSection = styled.section`
  ...
  &::before {
    ...
    background: radial-gradient(ellipse at 50% 100%, rgba(230,48,24,.07), transparent 70%);
    pointer-events: none;
  }
`;
  ...
  return (
    <>
      ...
      <Page>
        ...
        {/* ── STICKY NAV ── */}
        <NavBar>
          <NavBack onClick={goToEngineering}>← Engineering</NavBack>
          <NavTitle>
            <LogoMark size={16} colors={[T.bs, T.bsm, T.bsd]} />
            CARBO<span style={{ color: T.bs }}>NATION</span>
            <NavPill>BS Series</NavPill>
          </NavTitle>
          <Mono $size="7px" $color="#333">Consumer Carbon</Mono>
        </NavBar>
        ...
        <BsHeroWrap>
          ...
          <BsHeroRight>
            <BsHonest>
              <div>
                <span style={{ fontFamily: T.ffMono, fontSize: "7.5px", letterSpacing: ".14em", textTransform: "uppercase", color: T.bs, display: "block", marginBottom: 5 }}>
                  ⚠ Honest Statement
                </span>
                <span style={{ fontSize: 13, color: T.g4, lineHeight: 1.75, fontWeight: 300 }}>
                  BS Series parts do not improve performance, reduce lap times, or save meaningful weight. Genuine carbon fibre, applied cosmetically. We make them to the same standard as our engineering line because we physically cannot do it any other way.
                </span>
              </div>
            </BsHonest>

            <div style={{ marginTop: 32, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: "#1a0a06" }}>
              {[
                ...
                <div
                  key={p.id}
                  style={{ background: "#060606", padding: "22px 24px", transition: "background .2s, border-top-color .2s", position: "relative", overflow: "hidden", borderTop: `2px solid transparent` }}
        ...
        <BsGrid3>
          {BS_RE.map((p) => <BsProductCard key={p.id} product={p} />)}
        </BsGrid3>
        ...
        <BsGrid2>
          {BS_TR.map((p) => <BsProductCard key={p.id} product={p} />)}
        </BsGrid2>
        ...
        <BsGrid4>
          {BS_TECH.map((p) => <BsProductCard key={p.id} product={p} />)}
        </BsGrid4>
        ...
        <CtaSection>
          ...
          <CondHead $size="clamp(44px,7vw,96px)" $em={T.bs} style={{ marginBottom: 16, lineHeight: 0.86 }}>
        ...
        <Footer>
          <FooterInner>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <FooterName>CARBO<span style={{ color: T.bs }}>NATION</span> <span style={{ color: "#333", fontSize: 12 }}>BS</span></FooterName>
            </div>
