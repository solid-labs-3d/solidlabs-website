...
function LayerDiagram() {
  ...
  for (let i = 0; i < TOTAL; i++) {
    ...
    if (fromTop < TOP_SOLID) {
      layers.push({ type:'top', opacity: 0.7 + fromTop * 0.06 })
    } else if (i < BOT_SOLID) {
      layers.push({ type:'bot', opacity: 0.5 + i * 0.04 })
    } else {
      layers.push({ type:'mid' })
    }
  }
  // reverse so index 0 = bottom
  const reversed = [...layers].reverse()

  return (
    <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
      {reversed.map((l, i) => {
        if (l.type === 'top') return (
          <div key={i} style={{ height:10, background:`rgba(240,92,30,${l.opacity})`, borderRadius:1 }}/>
        )
        if (l.type === 'bot') return (
          <div key={i} style={{ height:10, background:`rgba(122,46,15,${l.opacity})`, borderRadius:1 }}/>
        )
        // mid = perimeter walls + infill
        return (
          <div key={i} style={{ height:10, display:'flex', gap:0 }}>
            <div style={{ width:14, background:'#c44820', opacity:.75, borderRadius:'1px 0 0 1px' }}/>
            <div style={{ flex:1, background:'#1e1e1e', border:'1px solid #2a2a2a' }}/>
            <div style={{ width:14, background:'#c44820', opacity:.75, borderRadius:'0 1px 1px 0' }}/>
          </div>
        )
      })}
    </div>
...
const PRINTER_PARTS = [
  {
    n:'01', name:'Extruder & Nozzle', role:'Melts & Deposits',
    ...
    icon:(
      <svg width="64" height="80" viewBox="0 0 72 80" fill="none">
        ...
        <line x1="20" y1="16" x2="52" y2="16" stroke="#f05c1e" strokeWidth="1.5" opacity=".5"/>
        <line x1="20" y1="22" x2="52" y2="22" stroke="#f05c1e" strokeWidth="1"   opacity=".3"/>
        ...
        <line x1="36" y1="68" x2="36" y2="78" stroke="#f05c1e" strokeWidth="3" strokeLinecap="round" opacity=".7"/>
      </svg>
    ),
  },
  {
    n:'02', name:'Build Plate', role:'Foundation',
    ...
    icon:(
      <svg width="80" height="60" viewBox="0 0 80 60" fill="none">
        ...
        <line x1="38" y1="20" x2="42" y2="20" stroke="#f05c1e" strokeWidth="1.5"/>
        <line x1="40" y1="18" x2="40" y2="22" stroke="#f05c1e" strokeWidth="1.5"/>
  ...
  {
    n:'03', name:'Motion System', role:'XYZ Positioning',
    ...
    icon:(
      <svg width="80" height="60" viewBox="0 0 80 60" fill="none">
        ...
        <text x="36" y="56" fontFamily="'DM Mono'" fontSize="7" fill="#444" letterSpacing=".1em">X·Y·Z</text>
      </svg>
    ),
  },
  {
    n:'04', name:'Filament Spool', role:'Raw Material',
    ...
    icon:(
      <svg width="70" height="70" viewBox="0 0 70 70" fill="none">
        ...
        <path d="M18 15 Q35 12 52 15" fill="none" stroke="#f05c1e" strokeWidth="3" opacity=".9"/>
        <path d="M18 21 Q35 18 52 21" fill="none" stroke="#c44820" strokeWidth="3" opacity=".7"/>
        <path d="M18 27 Q35 24 52 27" fill="none" stroke="#f05c1e" strokeWidth="3" opacity=".5"/>
        <path d="M18 33 Q35 30 52 33" fill="none" stroke="#c44820" strokeWidth="3" opacity=".35"/>
        <ellipse cx="35" cy="35" rx="6" ry="6" fill="#141414"/>
      </svg>
    ),
  },
  {
    n:'05', name:'Frame & Hardware', role:'Structure',
  ...
  {
    n:'06', name:'Control Board', role:'The Brain',
    ...
    icon:(
      <svg width="72" height="56" viewBox="0 0 76 60" fill="none">
        <rect x="6" y="10" width="64" height="40" rx="2" fill="#162016" stroke="#1e3a1e" strokeWidth="1.5"/>
        <polyline points="14,20 14,30 30,30 30,40" fill="none" stroke="#2a5a2a" strokeWidth="1.5"/>
        <polyline points="62,20 50,20 50,40"       fill="none" stroke="#2a5a2a" strokeWidth="1.5"/>
        <rect x="28" y="16" width="18" height="12" rx="1" fill="#1e1e1e" stroke="#333" strokeWidth="1"/>
        <rect x="30" y="18" width="14" height="8"  rx=".5" fill="#f05c1e" opacity=".7"/>
        <text x="31" y="24" fontFamily="'DM Mono'" fontSize="4.5" fill="rgba(0,0,0,.6)" letterSpacing=".05em">MCU</text>
      </svg>
    ),
  },
]
...
const FILAMENTS = [
  {
    name:'PLA+', tag:'Most Common', highlight:true,
    bars:[
...
const LAYER_LEGEND = [
  {
    ...
    style:{ borderColor:'#f05c1e' }, valColor:'#f05c1e',
  },
  {
  ...
  },
  {
    ...
    style:{ borderColor:'#333' }, valColor:'',
  },
  {
...
const PRINT_STEPS = [
  {
    ...
    icon:(
      <svg width="80" height="60" viewBox="0 0 80 60" fill="none">
        <rect x="34" y="4" width="12" height="36" rx="6" fill="none" stroke="#2a2a2a" strokeWidth="2"/>
        <rect x="36" y="24" width="8" height="18" rx="4" fill="#f05c1e" opacity=".8"/>
        ...
        <line x1="46" y1="26" x2="52" y2="26" stroke="#f05c1e" strokeWidth="1.5"/>
        <text x="54" y="29" fontFamily="'DM Mono'" fontSize="8" fill="#f05c1e">220°</text>
      </svg>
    ),
  },
  {
    n:'02', title:'First Layer',
    ...
    icon:(
      <svg width="80" height="60" viewBox="0 0 80 60" fill="none">
        ...
        <rect x="8" y="40" width="64" height="7" rx="1" fill="#f05c1e" opacity=".6"/>
        <polygon points="30,10 50,10 46,26 34,26" fill="#444"/>
        <polygon points="34,26 46,26 43,34 37,34" fill="#f05c1e"/>
        <line x1="8" y1="36" x2="72" y2="36" stroke="#2a2a2a" strokeWidth="1" strokeDasharray="4 3"/>
        <polygon points="72,33 72,39 78,36" fill="#333"/>
      </svg>
    ),
  },
  {
    n:'03', title:'Layer Stacking',
  ...
  },
  {
    ...
    icon:(
      <svg width="80" height="60" viewBox="0 0 80 60" fill="none">
        ...
        <rect x="20" y="18" width="40" height="6"  rx="1" fill="#f0ede6" opacity=".7"/>
        <line x1="40" y1="4" x2="40" y2="14" stroke="#888" strokeWidth="1.5" strokeDasharray="2 2"/>
        <polygon points="37,13 43,13 40,18" fill="#888"/>
        <line x1="28" y1="6" x2="32" y2="14" stroke="#666" strokeWidth="1" strokeDasharray="2 2"/>
        <line x1="52" y1="6" x2="48" y2="14" stroke="#666" strokeWidth="1" strokeDasharray="2 2"/>
        <text x="28" y="8" fontFamily="'DM Mono'" fontSize="6" fill="#555" letterSpacing=".05em">COOL</text>
      </svg>
    ),
  },
  {
    n:'05', title:'Part Release & QC',
    ...
    icon:(
      <svg width="80" height="50" viewBox="0 0 80 50" fill="none">
        ...
        <text x="56" y="24" fontFamily="'DM Mono'" fontSize="7" fill="#f05c1e">±0.2</text>
      </svg>
    ),
  },
]
...
const QUALITY_FACTORS = [
  {
    name:'Layer Height',
    lo:'0.05mm', mid:'0.20mm', hi:'0.40mm', barW:'55%',
  ...
  {
    name:'Print Speed',
    lo:'20mm/s', mid:'60mm/s', hi:'200mm/s', barW:'35%',
  ...
  {
    name:'Wall Count',
    lo:'2 walls', mid:'4 walls', hi:'8 walls', barW:'45%',
  ...
  {
    name:'Orientation',
    lo:'', mid:'Print orientation matters most', hi:'', barW:'80%',
  ...
  {
    name:'Temperature',
    lo:'185°C', mid:'215°C', hi:'260°C', barW:'50%',
    ...
    <div style={{ background: 'var(--blk)' }}>
      ...
      <section className="learn-hero">
        ...
        <h1 className="learn-h1 rv d1">
          HOW<br/><em>3D PRINTING</em><br/>WORKS.
        </h1>
        <p className="learn-sub rv d2">
          Everything about FDM printing — from machine parts to why your file becomes a solid object. Real mechanics, no fluff.
        </p>
      </section>
      ...
      <section className="sec-pad" style={{ background:'var(--blk)' }}>
        ...
        <div className="filament-chart">
          {FILAMENTS.map((f,i) => (
            <div key={f.name} className={`fil-col ${f.highlight ? 'highlight' : ''} rv d${i+1}`}>
              ...
              <div className="fil-bar-wrap">
                {f.bars.map(b => (
                  <div key={b.l} className="fil-bar-row">
                    <div className="fil-bar-label">{b.l}<span>{b.v}</span></div>
                    <div className="fil-bar-track">
                      <div className="fil-bar-fill" style={{ width:`${b.w}%`, background:b.c }}/>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="sec-pad">
        ...
        <div className="layer-infog rv">
          ...
          <div className="layer-visual">
            <div style={{ fontFamily:'var(--ff-mono)', fontSize:'8.5px', letterSpacing:'.18em', textTransform:'uppercase', color:'var(--g3)', marginBottom:20 }}>
              Cross-section · 20 layers shown · ↑ top skin stronger ↑
            </div>
            <LayerDiagram />
            <div style={{ display:'flex', justifyContent:'space-between', marginTop:14 }}>
              <span style={{ fontFamily:'var(--ff-mono)', fontSize:8, color:'var(--g3)', letterSpacing:'.1em' }}>↓ Base layer (bottom skin)</span>
              <span style={{ fontFamily:'var(--ff-mono)', fontSize:8, color:'var(--or)',  letterSpacing:'.1em' }}>↑ Top layer (top skin)</span>
            </div>
            <div style={{ display:'flex', gap:20, marginTop:18, flexWrap:'wrap' }}>
              {[
                { c:'#f05c1e', label:'Top/bottom skin (solid)' },
                { c:'#c44820', label:'Perimeter walls' },
                { c:'#1e1e1e', label:'Infill (gyroid 40%)', border:'1px solid #333' },
              ].map(lg => (
                <div key={lg.label} style={{ display:'flex', alignItems:'center', gap:7 }}>
                  <div style={{ width:24, height:10, background:lg.c, border:lg.border||'none' }}/>
                  <span style={{ fontFamily:'var(--ff-mono)', fontSize:8, color:'var(--g4)', letterSpacing:'.1em' }}>{lg.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="layer-legend">
            {LAYER_LEGEND.map(ll => (
              <div key={ll.title} className="ll-item" style={ll.style}>
                ...
                <div className="ll-value" style={ll.valColor ? { color:ll.valColor } : {}}>{ll.value}</div>
                <p className="ll-desc">{ll.desc}</p>
              </div>
            ))}
            {/* settings box */}
            <div style={{ background:'var(--s1)', padding:'18px 20px', borderLeft:'3px solid var(--or)', marginTop:4 }}>
              <div style={{ fontFamily:'var(--ff-mono)', fontSize:8, letterSpacing:'.18em', textTransform:'uppercase', color:'var(--g3)', marginBottom:12 }}>Our Standard Settings</div>
              {[
                ['Layer height',      '0.2mm'],
                ...
                <div key={k} style={{ display:'flex', justifyContent:'space-between', fontFamily:'var(--ff-mono)', fontSize:9, borderBottom: i<arr.length-1 ? '1px solid var(--s2)' : 'none', paddingBottom:i<arr.length-1?8:0, marginBottom:i<arr.length-1?8:0 }}>
          ...
          {QUALITY_FACTORS.map((q,i) => (
            <div key={q.name} className={`qf-item rv d${(i%3)+1}`}>
              ...
              <div className="qf-bar-wrap">
                <div style={{ fontFamily:'var(--ff-mono)', fontSize:8, letterSpacing:'.12em', textTransform:'uppercase', color:'var(--g3)', marginBottom:6, display:'flex', justifyContent:'space-between' }}>
                  <span>{q.lo}</span>
      ...
      <section className="sec-pad" style={{ background:'var(--s1)' }}>
        ...
        <p className="rv" style={{ fontSize:15, color:'var(--g5)', maxWidth:680, lineHeight:1.85, marginBottom:64 }}>
          A printed part is just the beginning. Post-processing determines whether your part looks raw or production-ready. We offer a full finishing pipeline — from sanding to spray-booth colour matching.
        </p>
        <div className="quality-grid">
          {FINISHING.map((f,i) => (
            <div key={f.title} className={`qf-item rv d${(i%3)+1}`}>
              <div style={{ width:32, height:3, background:'var(--or)', marginBottom:18 }}/>
      ...
      {/* ── CTA ── */}
      <section style={{ padding:'100px 48px', background:'var(--blk)', display:'flex', flexDirection:'column', alignItems:'center', textAlign:'center', gap:28, position:'relative', overflow:'hidden', borderTop:'1px solid var(--s2)' }}>
        <div style={{ position:'absolute', inset:0, background:'radial-gradient(ellipse 60% 80% at 50% 50%,rgba(240,92,30,.06),transparent 70%)' }}/>
        <h2 style={{ fontFamily:'var(--ff-cond)', fontWeight:900, fontSize:'clamp(44px,8vw,96px)', lineHeight:.88, position:'relative' }} className="rv">
          READY TO<br/><em style={{ fontStyle:'italic', color:'var(--or)' }}>PRINT?</em>
        </h2>
        <p style={{ fontSize:15, color:'var(--g5)', fontWeight:300, maxWidth:380, lineHeight:1.8, position:'relative' }} className="rv d1">
          Now you know how it works — let's build something together.
        </p>
        <div style={{ display:'flex', gap:16, position:'relative' }} className="rv d2">
      ...
      <footer style={{ background:'var(--blk)', borderTop:'1px solid var(--s2)' }}>
        <div className="site-footer">
          ...
          <div className="fl-copy">© 2025 SolidLabs · How 3D Printing Works · Bengaluru</div>
        </div>
      </footer>
    </div>
  )
